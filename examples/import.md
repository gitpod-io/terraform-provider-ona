...
Import one automation:

```shell
ONA_TOKEN="<personal-access-token>" go run ./scripts \
  -resource-type automation \
  -resource-id 019c6b3c-f5ab-7c2a-b7ee-07c6e29d3bff \
  -workdir ona-terraform-import-automation-019c6b3c \
  -refresh-import-map
```

Import one runner and its environment classes:
...
Import all custom groups:

```shell
ONA_TOKEN="<personal-access-token>" go run ./scripts \
  -resource-type group \
  -workdir ona-terraform-import-groups
```
...
Use `-resource-type` or `-resource-kind` to select resource kinds. Values can be Terraform type names or short names. The supported import types are:

- `automation` or `ona_automation`
- `group` or `ona_group`
- `organization_policy` or `ona_organization_policies`
- `project` or `ona_project`
- `runner` or `ona_runner`
- `runner_environment_class` or `ona_runner_environment_class`
- `security_policy` or `ona_security_policy`
- `team` or `ona_team`
...
The helper automatically adds selected dependencies used only for references:

- A selected runner pulls in that runner's environment classes.
- A selected project pulls in referenced runner environment classes and prebuild executor service accounts.
- A selected group pulls in service accounts referenced by group members.

The helper does not currently import `ona_ai_team_budget_policy` or `ona_ai_user_budget_policy`.

Automations are skipped in `imports.tf` when the Workflow API object contains features that `ona_automation` does not model. Current skip reasons are:

- reports
- incident triggers
- repository trigger contexts
...
Before writing files, the command cleans stale Terraform outputs in the work directory. It preserves `import-map.json`, then removes old generated `.tf`, `.tf.txt`, state, plan, lock, wrapper, and Terraform CLI configuration files.

Generated resources are split by type:

- `automations.tf`
- `groups.tf`
- `organization_policies.tf`
- `projects.tf`
- `runner_environment_classes.tf`
- `runners.tf`
...
- Project environment class IDs can become `ona_runner_environment_class.<name>.id`.
