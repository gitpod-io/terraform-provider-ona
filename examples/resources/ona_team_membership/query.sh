#!/usr/bin/env bash
# Inputs: ONA_TOKEN and TF_VAR_team_id; optional ONA_HOST, TMPDIR, and TERRAFORM
# from the environment. An optional output argument names a destination file.
# Example: ./query.sh generated-team_membership.tf
...
provider_dir=$(cd "$script_dir/../../.." >/dev/null && pwd)
...
workdir="$(mktemp -d "${TMPDIR:-/tmp}/ona-team_membership-query.XXXXXX")"
cleanup_dir="$workdir"
if [[ -z "$output" ]]; then
...
bin_dir="$workdir/bin"
mkdir -p "$bin_dir"
go -C "$provider_dir" build -trimpath -ldflags "-s -w -X main.version=dev" -o "$bin_dir/terraform-provider-ona" .
