#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")"; output="${1:-generated.tf}"; if [[ -e "$output" ]]; then echo "$output already exists" >&2; exit 1; fi
workdir="$(mktemp -d)"; trap 'rm -rf "$workdir"' EXIT; cp query.hcl "$workdir/query.tfquery.hcl"
