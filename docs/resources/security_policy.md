...
- `ports` (Block, Optional) Port admission policy for user-opened environment ports. (see [below for nested schema](#nestedblock--spec--ports))
...
- `max_admission_level` (String) Maximum admission level allowed for user-opened ports. Supported values are `everyone`, `organization`, and `creator_only`. Omit to leave the API default unset.
