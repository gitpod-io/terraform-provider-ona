...
func TestResourcesAreRegistered(t *testing.T) {
	...
	tests := []registrationTestCase{
		{Name: "automation_role_assignment", TypeName: "ona_automation_role_assignment", Expected: registrationExpectation{Count: 1}},
		...
		{Name: "github_app_integration", TypeName: "ona_github_app_integration", Expected: registrationExpectation{Count: 1}},
...
func TestRunnerTokenSchemaPersistsSensitiveToken(t *testing.T) {
	t.Parallel()

	provider := &OnaProvider{}
	for _, newResource := range provider.Resources(t.Context()) {
		resource := newResource()
		var metadataResp frameworkresource.MetadataResponse
		resource.Metadata(t.Context(), frameworkresource.MetadataRequest{ProviderTypeName: "ona"}, &metadataResp)
		if metadataResp.TypeName != "ona_runner_token" {
			continue
		}

		var schemaResp frameworkresource.SchemaResponse
		resource.Schema(t.Context(), frameworkresource.SchemaRequest{}, &schemaResp)
		attribute, ok := schemaResp.Schema.Attributes["token"]
		if !ok {
			t.Fatal("ona_runner_token schema is missing token")
		}
		if !attribute.IsSensitive() {
			t.Error("ona_runner_token.token must be sensitive")
		}
		if !attribute.IsComputed() {
			t.Error("ona_runner_token.token must be computed")
		}
		if attribute.IsWriteOnly() {
			t.Error("ona_runner_token.token must persist in state, not be write-only")
		}
		return
	}

	t.Fatal("ona_runner_token managed resource is not registered")
}
...
func TestManagedResourcesImplementImportState(t *testing.T) {
	...
	for _, newResource := range provider.Resources(t.Context()) {
		...
		resource.Metadata(t.Context(), frameworkresource.MetadataRequest{ProviderTypeName: "ona"}, &metadataResp)
		if metadataResp.TypeName == "ona_runner_token" {
			// Runner token issuance has no durable remote object or identity to import.
			continue
		}
...
func TestConfigureAllowsMissingToken(t *testing.T) {
	// not parallel: this test clears the process-wide ONA_TOKEN fallback.
	tests := []struct {
		Name     string
		EnvToken string
		Expected struct {
		...
		{Name: "without_token_defers_client_diagnostic", Expected: struct {
			HasError         bool
			HasProviderData  bool
			ClientConfigured bool
	...
	for _, tc := range tests {
		t.Run(tc.Name, func(t *testing.T) {
			...
			got := struct {
				...
				HasProviderData:  ok,
				ClientConfigured: ok && data.Client != nil,
			}

			if diff := cmp.Diff(tc.Expected, got); diff != "" {
				t.Errorf("Configure() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
