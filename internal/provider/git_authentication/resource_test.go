...
func TestValidateUpdateInput(t *testing.T) {
	...
	tests := []struct {
		...
		{Name: "accepts_unchanged_version_without_pat", PAT: types.StringNull()},
		{Name: "accepts_changed_version_with_pat", Rotated: true, PAT: types.StringValue("secret-pat")},
		{Name: "rejects_changed_version_with_null_pat", Rotated: true, PAT: types.StringNull(), Expected: Expectation{Err: "Missing Personal Access Token"}},
		{Name: "rejects_changed_version_with_unknown_pat", Rotated: true, PAT: types.StringUnknown(), Expected: Expectation{Err: "Missing Personal Access Token"}},
	}
	...
	for _, tc := range tests {
		t.Run(tc.Name, func(t *testing.T) {
			t.Parallel()
			var got Expectation
			diags := validateUpdateInput(tc.Rotated, tc.PAT)
			if diags.HasError() {
				got.Err = diags[0].Summary()
			}
			if diff := cmp.Diff(tc.Expected, got); diff != "" {
				t.Errorf("validateUpdateInput() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
