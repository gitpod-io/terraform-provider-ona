...
func (r *Resource) List(ctx context.Context, req list.ListRequest, resp *list.ListResultsStream) {
	resp.Results = func(push func(list.ListResult) bool) {
		...
		if integrationID != "" {
			...
			cache[integrationID] = integration
		...
		for listutil.HasCapacity(req.Limit, emitted) {
			...
			for _, token := range tokens {
				if !basicRepresentableToken(token, serviceAccountID, integrationID) {
					continue
				}
				integration, ok := cache[token.GetIntegrationId()]
				if !ok {
					integration, err = r.getSCMIntegration(ctx, token.GetIntegrationId())
					if err != nil {
						push(listutil.Error("Unable to Read Ona SCM Integration", err))
						return
					}
					cache[token.GetIntegrationId()] = integration
				}
				...
				if req.IncludeResource && !item.Diagnostics.HasError() {
					model := modelFromRemote(ctx, token, types.StringNull(), &item.Diagnostics)
			...
			paginationToken = result.Msg.GetPagination().GetNextToken()
			if paginationToken == "" {
				return
			}
		}
...
func basicRepresentableToken(token *v1.HostAuthenticationToken, serviceAccountID string, integrationID string) bool {
	if token == nil || token.GetId() == "" || token.GetSource() != v1.HostAuthenticationTokenSource_HOST_AUTHENTICATION_TOKEN_SOURCE_PAT || token.GetIntegrationId() == "" {
		return false
	}
	...
	return (serviceAccountID == "" || subject.GetId() == serviceAccountID) && (integrationID == "" || token.GetIntegrationId() == integrationID)
}

func representableToken(token *v1.HostAuthenticationToken, integration *v1.SCMIntegration) bool {
	return integration != nil && integration.GetPat() && integration.GetId() == token.GetIntegrationId() && integration.GetRunnerId() != "" && integration.GetHost() != "" && integration.GetRunnerId() == token.GetRunnerId() && integration.GetHost() == token.GetHost()
}
