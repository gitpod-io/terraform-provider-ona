...
func securityPolicySpecFromModel(ctx context.Context, model *SpecModel, root path.Path) (*v1.SecurityPolicy_Spec, diag.Diagnostics) {
	...
	if model.Ports != nil {
		validatePortPolicy(model.Ports, root.AtName("ports"), &diags)
	}
	if model.Executables != nil {
		spec.Executables = executablePolicyFromModel(model.Executables, root.AtName("executables"), &diags)
	}
	if model.Files != nil {
		validateFilePolicy(ctx, model.Files, root.AtName("files"), &diags)
	}
	if model.BlockDevices != nil {
		_ = effectFromString(model.BlockDevices.DefaultEffect, root.AtName("block_devices").AtName("default_effect"), &diags)
	}
	if model.Data != nil {
		validateDataPolicy(model.Data, root.AtName("data"), &diags)
	}
	return spec, diags
}

func validatePortPolicy(model *PortPolicyModel, root path.Path, diags *diag.Diagnostics) {
	_ = effectFromString(model.DefaultEffect, root.AtName("default_effect"), diags)
	for idx, rule := range model.Rules {
		...
		_ = effectFromString(rule.Effect, rulePath.AtName("effect"), diags)
	}
}
...
func validateFilePolicy(ctx context.Context, model *FilePolicyModel, root path.Path, diags *diag.Diagnostics) {
	_ = effectFromString(model.DefaultEffect, root.AtName("default_effect"), diags)
	if isKnownSet(model.DefaultActions) {
		actionDiags := validateFileActions(ctx, model.DefaultActions, root.AtName("default_actions"))
	...
	for idx, rule := range model.Rules {
		...
		if isKnownSet(rule.Actions) {
			actionDiags := validateFileActions(ctx, rule.Actions, rulePath.AtName("actions"))
			diags.Append(actionDiags...)
		}
	}
}

func validateDataPolicy(model *DataPolicyModel, root path.Path, diags *diag.Diagnostics) {
	_ = effectFromString(model.DefaultEffect, root.AtName("default_effect"), diags)
	for idx, rule := range model.Rules {
		rulePath := root.AtName("rule").AtListIndex(idx)
		_ = effectFromString(rule.Effect, rulePath.AtName("effect"), diags)
		if rule.Source == nil {
			...
			validateDataSource(rule.Source, rulePath.AtName("source"), diags)
		}
...
func validateDataSource(model *DataSourceModel, root path.Path, diags *diag.Diagnostics) {
...
func validateFileActions(ctx context.Context, set types.Set, attrPath path.Path) diag.Diagnostics {
	var diags diag.Diagnostics
	var values []string
