...
func TestAccGitAuthenticationQueryPaginatesAndBackfillsLimit(t *testing.T) {
	...
	server.service.hostAuthenticationTokens["git-auth-1-unsupported"] = gitAuthenticationQueryToken("git-auth-1-unsupported", "user-1", v1.Principal_PRINCIPAL_USER, v1.HostAuthenticationTokenSource_HOST_AUTHENTICATION_TOKEN_SOURCE_PAT, "runner-1", "github.com")
	server.service.hostAuthenticationTokens["git-auth-2"] = gitAuthenticationQueryToken("git-auth-2", "service-account-1", v1.Principal_PRINCIPAL_SERVICE_ACCOUNT, v1.HostAuthenticationTokenSource_HOST_AUTHENTICATION_TOKEN_SOURCE_PAT, "runner-1", "github.com")
...
func TestAccGitAuthenticationQueryRejectsRepeatedPaginationToken(t *testing.T) {
	t.Parallel()

	server := newGitAuthenticationQueryAPIServer(t)
	t.Cleanup(server.Close)
	server.service.hostAuthenticationNextPageToken = "repeated"

	testresource.UnitTest(t, QueryTestCase(server.URL, testresource.TestStep{
		Query:       true,
		Config:      gitAuthenticationQueryConfig(""),
		ExpectError: regexp.MustCompile(`ona API returned repeated pagination token "repeated"`),
	}))

	if got := len(server.service.hostAuthenticationListRequestsSnapshot()); got != 2 {
		t.Errorf("Git authentication list request count = %d, want 2", got)
	}
}

func TestAccGitAuthenticationQueryRejectsAmbiguousSCMIntegration(t *testing.T) {
	t.Parallel()

	server := newGitAuthenticationQueryAPIServer(t)
	t.Cleanup(server.Close)
	server.service.scmIntegrations["scm-3"] = &v1.SCMIntegration{Id: "scm-3", RunnerId: "runner-1", ScmId: "github", Host: "github.com", Pat: true}

	testresource.UnitTest(t, QueryTestCase(server.URL, testresource.TestStep{
		Query:       true,
		Config:      gitAuthenticationQueryConfig(""),
		ExpectError: regexp.MustCompile(`multiple PAT-enabled SCM integrations found for runner "runner-1" and host "github.com"`),
	}))
}
...
func newGitAuthenticationQueryAPIServer(t *testing.T) *runnerConfigurationAPIServer {
	...
	server.service.scmListOnlyIntegrations["static-scm-1"] = &v1.SCMIntegration{Id: "static-scm-1", RunnerId: "runner-3", ScmId: "bitbucket", Host: "bitbucket.org", Pat: true}
	server.service.hostAuthenticationTokens["git-auth-1"] = gitAuthenticationQueryToken("git-auth-1", "service-account-1", v1.Principal_PRINCIPAL_SERVICE_ACCOUNT, v1.HostAuthenticationTokenSource_HOST_AUTHENTICATION_TOKEN_SOURCE_PAT, "runner-1", "github.com")
	server.service.hostAuthenticationTokens["git-auth-user"] = gitAuthenticationQueryToken("git-auth-user", "user-1", v1.Principal_PRINCIPAL_USER, v1.HostAuthenticationTokenSource_HOST_AUTHENTICATION_TOKEN_SOURCE_PAT, "runner-1", "github.com")
	server.service.hostAuthenticationTokens["git-auth-oauth"] = gitAuthenticationQueryToken("git-auth-oauth", "service-account-1", v1.Principal_PRINCIPAL_SERVICE_ACCOUNT, v1.HostAuthenticationTokenSource_HOST_AUTHENTICATION_TOKEN_SOURCE_OAUTH, "runner-1", "github.com")
	server.service.hostAuthenticationTokens["git-auth-unlinked"] = gitAuthenticationQueryToken("git-auth-unlinked", "service-account-1", v1.Principal_PRINCIPAL_SERVICE_ACCOUNT, v1.HostAuthenticationTokenSource_HOST_AUTHENTICATION_TOKEN_SOURCE_PAT, "runner-missing", "github.com")
	server.service.hostAuthenticationTokens["git-auth-non-pat"] = gitAuthenticationQueryToken("git-auth-non-pat", "service-account-2", v1.Principal_PRINCIPAL_SERVICE_ACCOUNT, v1.HostAuthenticationTokenSource_HOST_AUTHENTICATION_TOKEN_SOURCE_PAT, "runner-2", "gitlab.com")
	server.service.hostAuthenticationTokens["git-auth-list-only"] = gitAuthenticationQueryToken("git-auth-list-only", "service-account-3", v1.Principal_PRINCIPAL_SERVICE_ACCOUNT, v1.HostAuthenticationTokenSource_HOST_AUTHENTICATION_TOKEN_SOURCE_PAT, "runner-3", "bitbucket.org")
	return server
}

func gitAuthenticationQueryToken(id string, subjectID string, principal v1.Principal, source v1.HostAuthenticationTokenSource, runnerID string, host string) *v1.HostAuthenticationToken {
