...
func (r *GroupMembershipResource) IdentitySchema(ctx context.Context, req resource.IdentitySchemaRequest, resp *resource.IdentitySchemaResponse) {
	resp.IdentitySchema = identityschema.Schema{Attributes: map[string]identityschema.Attribute{
		...
		"service_account_id": identityschema.StringAttribute{OptionalForImport: true, Description: "Service account member ID. Set exactly one member ID."},
		"user_id":            identityschema.StringAttribute{OptionalForImport: true, Description: "User member ID. Set exactly one member ID."},
	}}
}
