...
func TestPortAndServiceAuthenticationHeaders(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		ExpectedHeaders Expectation
	}{
		{
			Name: "restricted_port_sends_both_credentials",
			Interceptors: []connect.Interceptor{
				WithBearerToken("environment-token"),
				WithPortAccessToken("port-token"),
			},
			ExpectedHeaders: Expectation{Authorization: "Bearer environment-token", PortAuthorization: "port-token"},
		},
		{
			Name: "public_fallback_sends_only_service_credential",
			Interceptors: []connect.Interceptor{
				WithBearerToken("environment-token"),
			},
			ExpectedHeaders: Expectation{Authorization: "Bearer environment-token"},
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.Name, func(t *testing.T) {
			...
			client := v1connect.NewEnvironmentServiceClient(server.Client(), server.URL, connect.WithInterceptors(tc.Interceptors...))
			_, err := client.GetEnvironment(t.Context(), connect.NewRequest(&v1.GetEnvironmentRequest{EnvironmentId: "env-123"}))
			requestHeaders := <-headers
			got := Expectation{
				Authorization:     requestHeaders.Get(AuthorizationHeader),
				PortAuthorization: requestHeaders.Get(PortAuthorizationHeader),
			}
			if err != nil {
				got.Err = err.Error()
			}
			if diff := cmp.Diff(tc.ExpectedHeaders, got); diff != "" {
				t.Errorf("authentication headers mismatch (-want +got):\n%s", diff)
			}
		})
	}
}

func TestPortAndServiceAuthenticationStreamingHeaders(t *testing.T) {
	t.Parallel()

	connection := &recordingStreamingClientConn{requestHeader: make(http.Header)}
	...
	for index := len(interceptors) - 1; index >= 0; index-- {
		next = interceptors[index].WrapStreamingClient(next)
	}
	next(t.Context(), connect.Spec{IsClient: true})

	expected := http.Header{
		"Authorization":       []string{"Bearer environment-token"},
		"X-Ona-Authorization": []string{"port-token"},
	}
	if diff := cmp.Diff(expected, connection.requestHeader); diff != "" {
		t.Errorf("streaming authentication headers mismatch (-want +got):\n%s", diff)
	}
}
...
func (*recordingStreamingClientConn) Spec() connect.Spec           { return connect.Spec{} }
func (*recordingStreamingClientConn) Peer() connect.Peer           { return connect.Peer{} }
func (*recordingStreamingClientConn) Send(any) error               { return nil }
func (c *recordingStreamingClientConn) RequestHeader() http.Header { return c.requestHeader }
func (*recordingStreamingClientConn) CloseRequest() error          { return nil }
func (*recordingStreamingClientConn) Receive(any) error            { return nil }
func (*recordingStreamingClientConn) ResponseHeader() http.Header  { return nil }
func (*recordingStreamingClientConn) ResponseTrailer() http.Header { return nil }
func (*recordingStreamingClientConn) CloseResponse() error         { return nil }
...
func TestOnaUserAgent(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			Name: "raw_client_request",
			Expected: Expectation{
				Header: "ona-go-client/" + version + " (language=go; layer=raw)",
			},
		},
		{
			Name:     "sdk_layer_request",
			SDKLayer: true,
			Expected: Expectation{
				Header: "ona-go-sdk/" + version + " (language=go; layer=sdk)",
			},
		},
		{
			Name:   "raw_client_request_with_application_prefix",
			Prefix: "my-app/1.2",
			Expected: Expectation{
				Header: "my-app/1.2 ona-go-client/" + version + " (language=go; layer=raw)",
			},
		},
		{
			Name:     "sdk_layer_request_with_application_prefix",
			Prefix:   "my-app/1.2",
			SDKLayer: true,
			Expected: Expectation{
				Header: "my-app/1.2 ona-go-sdk/" + version + " (language=go; layer=sdk)",
			},
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.Name, func(t *testing.T) {
			...
			ctx := t.Context()
			if tc.SDKLayer {
				ctx = withSDKUserAgentLayer(ctx)
			}

			var got Expectation
			conn := withOnaUserAgent(tc.Prefix).WrapStreamingClient(func(ctx context.Context, spec connect.Spec) connect.StreamingClientConn {
				return &fakeStreamingClientConn{requestHeader: http.Header{}}
			})(ctx, connect.Spec{IsClient: true})
			got.Header = conn.RequestHeader().Get(UserAgentHeader)

			if diff := cmp.Diff(tc.Expected, got); diff != "" {
				t.Errorf("withOnaUserAgent() mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
...
func (c *fakeStreamingClientConn) Spec() connect.Spec {
	return connect.Spec{}
}

func (c *fakeStreamingClientConn) Peer() connect.Peer {
	return connect.Peer{}
}

func (c *fakeStreamingClientConn) Send(any) error {
	return nil
}
...
func (c *fakeStreamingClientConn) CloseRequest() error {
	return nil
}
...
func (c *fakeStreamingClientConn) ResponseTrailer() http.Header {
	return http.Header{}
}
